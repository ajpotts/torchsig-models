"""Training entry points for EfficientNet-2D classifiers on TorchSig spectrogram datasets.

This module loads TorchSig dataset configurations, prepares train/validation/test
data loaders, builds an EfficientNet-2D classifier, and trains it with the shared
...
EfficientNet2DModelName = Literal[
    "efficientnet_b0",
    "efficientnet_b2",
    "efficientnet_b4",
]
...
def load_training_params(
    model_name: EfficientNet2DModelName,
...
def _spectrogram_transforms(
    cfg: TorchSigDatasetConfig,
) -> list[Any]:
    """Build transforms for EfficientNet-2D spectrogram training.

    Args:
        cfg: TorchSig dataset configuration containing the FFT settings.
...
def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for EfficientNet-2D training.
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "unless split-specific configs are provided."
        ),
    )
