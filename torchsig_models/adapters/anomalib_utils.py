...
class TorchSigAnomalibDataset(AnomalibDataset):
    ...
    def __init__(
        ...
        self.samples = pd.DataFrame(
            {
                "image_path": [str(self.root)]
                * num_samples,  # placeholder path (exists on disk)
                "split": [split] * num_samples,  # split information
                "label_index": [LabelName.UNKNOWN]
                * num_samples,  # determine labels at runtime
    ...
    def __getitem__(self, index: int) -> ImageItem:
        ...
        data_element = self.dataset[
            self.index_bounds[0] + index
        ]  # map index to current split

        # Process TorchSig data element
        anomalies = []
        if isinstance(data_element, tuple):
            spec_np, labels = (
                data_element  # data, list of target_labels list (each signal)
            )
            ...
            if self.anomaly_label_index is not None:
                if isinstance(
                    labels[0], list
                ):  # multiple Signals present in element (list of lists)
                    anomaly_labels: list = labels[
                        self.anomaly_label_index
                    ]  # list of np.bool_
                    for signal_label in anomaly_labels:
                        if bool(signal_label):
                            anomalies = [
                                True
                            ]  # classify image as anomaly if 1+ anomalies present
        ...
        if self.samples.attrs["task"] == "segmentation":  # NOT SUPPORTED
            ...
            if self.use_anomaly_labels and len(anomalies) > 0:
                gt_mask = Mask(
                    torch.ones((H, W), dtype=torch.uint8)
                )  # all pixels anomalies
...
class TorchSigAnomalibDataModule(AnomalibDataModule):
    ...
    def __init__(
        ...
        self.dataset_sizes[2] = (
            total_size - self.dataset_sizes[0] - self.dataset_sizes[1]
        )

        # dataset index [min, max] for train/val/test
        self.index_bounds[0] = [0, self.dataset_sizes[0] - 1]
        self.index_bounds[1] = [
            self.index_bounds[0][1] + 1,
            self.index_bounds[0][1] + self.dataset_sizes[1],
        ]
        self.index_bounds[2] = [
            self.index_bounds[1][1] + 1,
            self.index_bounds[1][1] + self.dataset_sizes[2],
        ]
...
class SpectrogramZoom(SignalTransform):
    ...
    def __init__(self, output_shape: tuple[int, int] = (256, 256), **kwargs):
    ...
    def __apply__(self, signal: Signal) -> Signal:
        ...
        signal.data = zoom(
            signal.data,
            zoom=(
                self.output_shape[0] / in_shape[0],
                self.output_shape[1] / in_shape[1],
            ),
            order=2,
            mode="nearest",
            prefilter=True,
...
class SpectrogramRescale(SignalTransform):
    ...
    def __init__(
        self, clip_db_range: tuple[float, float] | None, eps: float = 1e-8, **kwargs
    ):
